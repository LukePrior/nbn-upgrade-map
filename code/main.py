...
def get_address(nbn: NBNApi, address: Address, get_status=True) -> Address:
    ...
    try:
        ...
        if address.loc_id and get_status:
            ...
            address.techChangeStatus = status["addressDetail"].get("techChangeStatus", "UNKNOWN")
            address.programType = status["addressDetail"].get("programType", "UNKNOWN")
            address.targetEligibilityQuarter = status["addressDetail"].get("targetEligibilityQuarter", "UNKNOWN")
